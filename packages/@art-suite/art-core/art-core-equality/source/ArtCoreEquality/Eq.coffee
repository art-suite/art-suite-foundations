{remove}              = require '@art-suite/art-core-arrays'
{objectKeyCount}      = require '@art-suite/art-core-objects'
{floatTrue0}          = require '@art-suite/art-core-math'
{isString, isNumber, isDate}  = require '@art-suite/art-core-types'
{min} = Math
...
compareArray = (a, b, recursionBlockArray, compareFunctionsAsStrings) =>
  aLength = a.length
  bLength = b.length
  ...
  for i in [0...Math.min aLength, bLength] by 1
    av = a[i]
    bv = b[i]
    return val if 0 != val = compare av, bv, recursionBlockArray, compareFunctionsAsStrings

  aLength - bLength

# recursively compares all properties in both a and b
# If all are equal, returns a.length - b.length
compareObject = (a, b, recursionBlockArray, compareFunctionsAsStrings) =>
  aLength = 0
  compared = 0
...
compare = (a, b, recursionBlockArray, compareFunctionsAsStrings) =>
  ...
  if a? && b? && a.constructor == _constructor = b.constructor
    ...
    return compareArray  a, b, recursionBlockArray, compareFunctionsAsStrings if _constructor == Array
    return compareObject a, b, recursionBlockArray, compareFunctionsAsStrings if _constructor == Object
